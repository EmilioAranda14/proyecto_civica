...
-- Coste de envío por unidad dentro de cada pedido
...
-- Resultado final: shipping + promo a nivel de línea
final AS (
    SELECT
        oi.order_items_id,
        oi.order_id,
        ...
        (oi.quantity * scp.shipping_cost_unit)::NUMBER(16,4)    AS shipping_cost_line,

        -- PROMO
        COALESCE(ppu.promo_amount_order, 0)::NUMBER(12,2)                   AS promo_amount_order,
        COALESCE(ppu.promo_amount_unit, 0)                                  AS promo_amount_unit,
        (oi.quantity * COALESCE(ppu.promo_amount_unit, 0))::NUMBER(16,4)    AS promo_amount_line
